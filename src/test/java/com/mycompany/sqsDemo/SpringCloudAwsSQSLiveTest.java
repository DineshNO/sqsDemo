...
import static com.mycompany.sqsDemo.UserEventListeners.EVENT_TYPE_CUSTOM_HEADER;
...
public class SpringCloudAwsSQSLiveTest {
    ...
    void givenAStringPayload_whenSend_shouldReceive() {
        ...
        // when
        sqsTemplate.send(to -> to.queue(eventQueuesProperties.getUserCreatedByNameQueue()).payload(userName));
        logger.info("Message sent with payload {}", userName);

        // then
        await().atMost(Duration.ofSeconds(3)).until(() -> userRepository.findByName(userName).isPresent());
    }
    ...
    void givenARecordPayload_whenSend_shouldReceive() {
        ...
        await().atMost(Duration.ofSeconds(3)).until(() -> userRepository.findById(userId).isPresent());
    }
    ...
    void givenCustomHeaders_whenSend_shouldReceive() {
        ...
        // when
        sqsTemplate.send(to -> to.queue(eventQueuesProperties.getUserCreatedEventTypeQueue()).payload(payload).headers(headers));
